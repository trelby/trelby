...
class ScriptReport:
    ...
    def generate(self) -> bytes:
        ...
        tf.addText(
            _("Max / avg. scene length in lines")
            + ": %d / %.2f" % (self.sr.longestScene, self.sr.avgScene)
        )
        ...
        tf.addText(
            _("Max / avg. action element length in lines")
            + ": %d / %.2f" % (maxA, avgA)
        )

        tf.addSpace(4.0)

        tf.addText(_("Speaking characters") + ": %d" % len(self.cr.cinfo))
